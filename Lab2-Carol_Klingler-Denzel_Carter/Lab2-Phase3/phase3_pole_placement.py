...
def calculate_gains_from_poles(A, B, desired_poles):
    ...
    mags = np.abs(np.asarray(desired_poles))
    K = np.diag(mags)  # REPLACE THIS
...
class PolePlacementController:
    ...
    def update(self, x_ref, y_ref, z_ref, x_meas, y_meas, z_meas):
        ...
        x = np.array([float(x_meas), float(y_meas), float(z_meas)], dtype=float)
        
        # TODO: Form reference vector
        x_ref_vec = np.array([float(x_ref), float(y_ref), float(z_ref)], dtype=float)
        
        # TODO: Calculate error
        error = x - x_ref_vec  # (x - x_ref)
        
        # TODO: Apply control law u = -K * error
        u = -self.K @ error  # = K * (x_ref - x)
