    ...
    while True:
        ...
        if pose['detected'] and not attack_triggered:
            ...
            if attack_triggered:
                ...
                # Tag briefly lost but attack not yet triggered:
                # treat as normal KF on real ToF (no horizontal update)
                phase = 'approach'
                kf_mode = 'normal'
                z_tof_reported = z_tof_real
                kf.predict(u=0.0)
        ...
        if reached_horizontal_target and not reached_vertical_target:
            dz = target_z - z_est
            print(f"[DEBUG] dz={dz:.3f}, z_est={z_est:.3f}, z_tof_real={z_tof_real:.3f}, z_tof_reported={z_tof_reported:.3f}")
