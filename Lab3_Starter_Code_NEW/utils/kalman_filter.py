...
class AltitudeKalmanFilter:
    ...
    def predict(self, u):
        ...
        #
        # x_{k|k-1} = predicted state at step k
        # A = state transition matrix
        # B = input matrix
        # u_{k-1} = acceleration command at previous step
        ...
        #
        # P_{k|k-1} = predicted covariance at step k
        # P_{k-1|k-1} = previous covariance
    ...
    def update(self, measurement):
        ...
        #
        # nu = measurement residual (difference between actual and predicted measurement)
        # y = actual measurement
        # H * x_{k|k-1} = predicted measurement from current state estimate
        ...
        #
        # S = variance of the innovation
        # P_{k|k-1} = predicted state covariance
        ...
        #
        # K = Kalman gain (how much we trust the measurement vs prediction)
        self.K = self.P @ self.H.T @ np.linalg.inv(S)
        
        # 4. State update: x_{k|k} = x_{k|k-1} + K * nu
        #    Update self.x_hat using Kalman gain and innovation
        #
        # x_{k|k} = corrected state estimate after seeing the measurement
        ...
        #    Hint: I = np.eye(2)
        #
        # P_{k|k} = corrected covariance (uncertainty after update)
        I = np.eye(2)
...
def test_kalman_filter():
    ...
    kf_normal = AltitudeKalmanFilter(dt=0.1)
    ...
    kf_attack = AltitudeKalmanFilter(dt=0.1)
